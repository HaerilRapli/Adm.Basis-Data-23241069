...
-- 1. Menampilkan daftar nama semua customer
SELECT nama_customer FROM customer;
-- 2. Menampilkan nama game dan kategori game, untuk mengetahui jenis game apa saja yang tersedia.
SELECT nama_game, kategori FROM game;
-- 3. Menampilkan nama item dan harga, untuk melihat item apa yang bisa dibeli beserta harganya.
SELECT nama_item, harga_item FROM item;
-- 4.  Melihat seluruh data transaksi yang pernah dilakukan, termasuk id customer dan total harga.
SELECT * FROM transaksi;
-- 5. Menampilkan nama customer dan metode pembayaran dengan penulisan prefix dan alias.
SELECT c.nama_customer, c.metode_pembayaran FROM customer AS c;
-- 6. Liat semua metode pembayaran yang dipake customer.
SELECT metode_pembayaran FROM customer;
-- 7. Ngubah nama kolom jadi lebih singkat dan jelas mengunakan alias.
SELECT nama_game AS Game, kategori AS Jenis FROM game;
-- 8. Pakai alias untuk tabel + prefix
-- abel item diganti alias i, terus kolomnya dipanggil pakai prefix i.
SELECT i.nama_item, i.harga_item FROM item AS i;
-- 9. Tampilkan nama game dari tabel game pakai prefix lengkap
SELECT top_up_game_online.game.nama_game FROM game;
-- 10. Tampilkan semua data dari tabel game pakai alias
